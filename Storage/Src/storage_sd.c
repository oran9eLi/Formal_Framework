...
static void Storage_SD_SetState(Px4Lite_State_t state,
                                uint32_t now_ms)
{
...
static Px4Lite_Result_t Storage_SD_WriteRaw(FIL *file,
                                            const char *line)
{
    UINT written = 0U;
    UINT length;
...
static Px4Lite_Result_t Storage_SD_OpenFile(FIL *file,
                                            const char *path,
                                            const char *header,
...
static Px4Lite_Result_t Storage_SD_OpenFiles(void)
{
    ...
    if (Storage_SD_OpenFile(&s_data_file,
                            "0:/" STORAGE_SENSOR_DATA_FILE,
                            StorageCsv_DataHeader(),
                            STORAGE_OPEN_PHASE_DATA_OPEN,
                            STORAGE_OPEN_PHASE_DATA_HEADER,
                            STORAGE_OPEN_PHASE_DATA_SEEK) != PX4LITE_OK)
    {
        return PX4LITE_IO_ERROR;
    }

    if (Storage_SD_OpenFile(&s_error_file,
                            "0:/" STORAGE_SYSTEM_ERROR_FILE,
                            StorageCsv_ErrorHeader(),
                            STORAGE_OPEN_PHASE_ERROR_OPEN,
                            STORAGE_OPEN_PHASE_ERROR_HEADER,
                            STORAGE_OPEN_PHASE_ERROR_SEEK) != PX4LITE_OK)
    {
        (void)f_close(&s_data_file);
        return PX4LITE_IO_ERROR;
    }
...
void Storage_SD_Service(uint32_t now_ms)
{
    ...
    if ((s_status.state == PX4LITE_STATE_ONLINE) ||
        ((s_status.last_attempt_ms != 0U) &&
         ((uint32_t)(now_ms - s_status.last_attempt_ms) <
          STORAGE_MOUNT_RETRY_MS)))
    {
        return;
    }
...
static Px4Lite_Result_t Storage_SD_WriteLine(FIL *file,
                                             const char *line,
                                             uint32_t now_ms)
{
...
{
    return ((s_status.state == PX4LITE_STATE_ONLINE) &&
            (s_status.mounted != 0U) &&
            (s_status.files_open != 0U))
               ? 1U
               : 0U;
