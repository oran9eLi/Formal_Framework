...
static uint8_t SdSpi_TransferByte(uint8_t value)
{
    ...
    (void)BSP_SPI1_TransmitReceive(&value,
                                   &rx,
                                   1U,
                                   STORAGE_SPI_TIMEOUT_MS);
    return rx;
}
...
static uint8_t SdSpi_WaitReady(uint32_t timeout_ms)
{
    ...
    do
    {
        if (SdSpi_TransferByte(SD_DUMMY_BYTE) == SD_DUMMY_BYTE)
        {
            return 1U;
        }
...
{
    BSP_SPI1_SD_Select();
    if (SdSpi_WaitReady(SD_CMD_TIMEOUT_MS) != 0U)
    {
        return 1U;
    }
    SdSpi_Deselect();
...
static uint8_t SdSpi_ReadBlock(uint8_t *buffer, uint32_t count)
{
    ...
    do
    {
        ...
        {
            if (BSP_SPI1_Receive(buffer,
                                 (uint16_t)count,
                                 STORAGE_SPI_TIMEOUT_MS) != BSP_STATUS_OK)
            {
                return 0U;
            }
            (void)SdSpi_TransferByte(SD_DUMMY_BYTE);
    ...
    } while ((uint32_t)(BSP_Time_GetTickMs() - start_ms) <
             SD_TOKEN_TIMEOUT_MS);
...
static uint8_t SdSpi_WriteBlock(const uint8_t *buffer)
{
    ...
    response = SdSpi_TransferByte(SD_DUMMY_BYTE);
    if ((response & SD_DATA_ACCEPTED_MASK) != SD_DATA_ACCEPTED)
    {
        return 0U;
    }
...
DSTATUS disk_initialize(BYTE pdrv)
{
    ...
    if (response == 1U)
    {
        for (i = 0U; i < 4U; ++i)
        {
            ocr[i] = SdSpi_TransferByte(SD_DUMMY_BYTE);
        }
        SdSpi_Deselect();
        ...
        SdSpi_Deselect();
        s_card_type = (uint8_t)(SD_TYPE_SD2 |
                                ((ocr[0] & 0x40U) != 0U
                                     ? SD_TYPE_BLOCK
                                     : 0U));
    }
...
DRESULT disk_read(BYTE pdrv, BYTE *buff, DWORD sector, UINT count)
{
    ...
    if ((s_card_type & SD_TYPE_BLOCK) == 0U)
    {
    ...
    while (count > 0U)
    {
        if ((SdSpi_SendCommand(SD_CMD17, sector) != 0U) ||
            (SdSpi_ReadBlock(buff, SD_BLOCK_SIZE) == 0U))
        {
            SdSpi_Deselect();
            return RES_ERROR;
        }
        SdSpi_Deselect();
        buff += SD_BLOCK_SIZE;
        sector += ((s_card_type & SD_TYPE_BLOCK) != 0U)
                      ? 1U
                      : SD_BLOCK_SIZE;
        count--;
    }
...
DRESULT disk_write(BYTE pdrv, const BYTE *buff, DWORD sector, UINT count)
{
    ...
    while (count > 0U)
    {
        if ((SdSpi_SendCommand(SD_CMD24, sector) != 0U) ||
            (SdSpi_WriteBlock(buff) == 0U))
        {
            SdSpi_Deselect();
            return RES_ERROR;
        }
...
DRESULT disk_ioctl(BYTE pdrv, BYTE cmd, void *buff)
{
    ...
    {
    case CTRL_SYNC:
        return (SdSpi_Select() != 0U) ? (SdSpi_Deselect(), RES_OK)
                                      : RES_ERROR;
