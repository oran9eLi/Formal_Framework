...
- 完成度口径：以 `Development_Guide/05_完成度与后续清单.md` 为准
...
当前真实完成度以 `Development_Guide/05_完成度与后续清单.md` 为唯一来源。不要仅凭文件、任务或函数声明存在就判断模块已经完成。
...
| `Development_Guide/05_完成度与后续清单.md` | 当前完成度、占位项、后续阶段和验收标准 |
...
6. 在 `Development_Guide/05_完成度与后续清单.md` 和 `Change_History/` 中记录完成度、验证结果和未完成项。
7. 如果代码框架、任务模型、接口边界或外设接入方式变化，必须同步修改 `Development_Guide/` 当前编号手册。
