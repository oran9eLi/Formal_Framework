...
v1.0 M1 起，四路目标完整校验后整体提交，任一路被拒绝时四路目标、时间戳和控制模式均保持不变。31011 保持当前模式；31013（四路微秒脉宽）在接受目标时同时选择 DIRECT。用户缓降中的全零/最小脉宽命令确认已有撤收意图，保持当前曲线与模式，不强制切换模式。验证见 [M1 记录](../Release/v1.0_M1_VALIDATION.md)。
