...
static __inline uint8_t Integrity_IsFresh(const Integrity_MeasurementHeader_t *header, uint32_t now_ms, uint32_t max_age_ms)
{
  if ((header == 0) || (header->valid == 0U)) { return 0U; }

  return ((uint32_t)(now_ms - header->publish_time_ms) <= max_age_ms) ? 1U : 0U;
}
