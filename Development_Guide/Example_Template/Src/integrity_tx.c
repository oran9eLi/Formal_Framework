    ...
    uint32_t timeout_ms)
{
    ...
    if ((context->state == INTEGRITY_TX_SENDING) ||
    ...
    uint32_t now_ms)
{
    ...
    if (context->state == INTEGRITY_TX_WAIT_COMPLETE)
    {
        return INTEGRITY_BUSY;
    }

    remaining = (uint16_t)(context->frame_length -
                           context->offset);
    written = write_fn(&context->frame[context->offset],
                       remaining);
    ...
    context->offset =
        (uint16_t)(context->offset + (uint16_t)written);

    if (context->offset < context->frame_length)
    {
        return INTEGRITY_BUSY;
    }
