...
Integrity_Result_t Integrity_SnapshotPublish(const Integrity_ExampleSnapshot_t *source)
{
  uint32_t primask;

  if (source == 0) { return INTEGRITY_INVALID_PARAM; }
...
Integrity_Result_t Integrity_SnapshotCopy(Integrity_ExampleSnapshot_t *destination)
{
  uint32_t primask;

  if (destination == 0) { return INTEGRITY_INVALID_PARAM; }
