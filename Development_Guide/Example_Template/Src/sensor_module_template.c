...
extern Integrity_Result_t BSP_SensorTemplate_ReadRaw(
    int32_t *raw_a,
    int32_t *raw_b,
...
Integrity_Result_t SensorTemplate_Service(uint32_t now_ms)
{
    ...
    result = BSP_SensorTemplate_ReadRaw(
        &raw_a,
        &raw_b,
        &sample_time_ms);
    ...
    {
        if ((uint32_t)(now_ms - s_last_rx_ms) >
            SENSOR_TEMPLATE_OFFLINE_MS)
        {
            s_state = COMPONENT_OFFLINE;
        }
