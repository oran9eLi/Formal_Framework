    ...
    const Integrity_HighRateSample_t *sample)
{
    ...
    {
        s_fifo.tail = (uint16_t)((s_fifo.tail + 1U) %
                                 INTEGRITY_FIFO_CAPACITY);
        s_fifo.count--;
    ...
    s_fifo.head = (uint16_t)((s_fifo.head + 1U) %
                             INTEGRITY_FIFO_CAPACITY);
    s_fifo.count++;
    s_fifo.stats.push_count++;
    s_fifo.stats.current_count = s_fifo.count;

    if (s_fifo.count > s_fifo.stats.peak_count)
    {
        s_fifo.stats.peak_count = s_fifo.count;
    }
