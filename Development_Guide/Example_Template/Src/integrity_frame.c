...
uint16_t Integrity_FrameEncode(uint8_t message_type,
                               uint32_t sequence,
                               ...
                               uint16_t capacity)
{
    ...
    frame_length = (uint16_t)(INTEGRITY_FRAME_HEADER_SIZE +
                              payload_length +
                              INTEGRITY_FRAME_CRC_SIZE);

    if ((output == 0) ||
        ((payload == 0) && (payload_length != 0U)) ||
        (payload_length > INTEGRITY_FRAME_PAYLOAD_MAX) ||
    ...
    {
        memcpy(&output[INTEGRITY_FRAME_HEADER_SIZE],
               payload,
               payload_length);
    }

    crc = Integrity_Crc16(output,
                          (uint16_t)(frame_length -
                                     INTEGRITY_FRAME_CRC_SIZE));
...
Integrity_Result_t Integrity_FrameParserFeed(
    Integrity_FrameParser_t *parser,
    uint8_t byte,
    Integrity_DecodedFrame_t *frame)
{
    uint16_t payload_length;
    ...
    if ((parser->position == 0U) &&
        (byte != INTEGRITY_FRAME_MAGIC_0))
    {
        return INTEGRITY_IDLE;
    }

    if ((parser->position == 1U) &&
        (byte != INTEGRITY_FRAME_MAGIC_1))
    {
        parser->position =
            (byte == INTEGRITY_FRAME_MAGIC_0) ? 1U : 0U;
    ...
    if (parser->position == 6U)
    {
        ...
        if ((parser->buffer[2] != INTEGRITY_FRAME_VERSION) ||
            (payload_length > INTEGRITY_FRAME_PAYLOAD_MAX))
        {
            parser->position = 0U;
            parser->format_error_count++;
        ...
        parser->expected_length =
            (uint16_t)(INTEGRITY_FRAME_HEADER_SIZE +
                       payload_length +
                       INTEGRITY_FRAME_CRC_SIZE);
    }

    if ((parser->expected_length == 0U) ||
        (parser->position < parser->expected_length))
    {
        return INTEGRITY_IDLE;
    }

    received_crc = Integrity_GetU16(
        &parser->buffer[parser->expected_length - 2U]);
    calculated_crc = Integrity_Crc16(
        parser->buffer,
        (uint16_t)(parser->expected_length - 2U));
    ...
    if (payload_length != 0U)
    {
        memcpy(frame->payload,
               &parser->buffer[INTEGRITY_FRAME_HEADER_SIZE],
               payload_length);
    }
