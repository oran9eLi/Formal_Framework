...
- `Development_Guide/07_移植进度与功能清单.md` 是模块完成度的唯一事实来源。
...
- 示例：`docs: 补充注释规范`。
- 架构、协议、日志、存储、显示或任务边界发生变化时，应同步更新 `Development_Guide/` 或 `Change_History/`。
...
| `Development_Guide/07_移植进度与功能清单.md` | 模块完成度事实来源 |
| `Development_Guide/10_Debug模块化使用指南.md` | Debug 开关系统 |
| `Development_Guide/12_团队模块化开发手册.md` | 新模块接入流程 |
| `Development_Guide/16_模块接入标准与评审清单.md` | GNSS/LoRa 模板和模块接入检查清单 |
