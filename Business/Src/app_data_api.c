                                  ...
                                  uint32_t now_ms)
{
    ...
    if ((Px4Lite_CopyAlarmSnapshot(&alarm) == PX4LITE_OK) &&
        (Px4Lite_IsFresh(&alarm.header,
                         now_ms,
                         APP_ALARM_MAX_AGE_MS) != 0U))
    {
    ...
    uint32_t now_ms)
{
    ...
    if (Px4Lite_CopyNavigation(&source) != PX4LITE_OK)
    {
    ...
    uint32_t now_ms)
{
    ...
    memset(out, 0, sizeof(*out));
    for (attempt = 0U;
         attempt < APP_STATUS_COPY_RETRY_MAX;
         ++attempt)
    {
            ...
            (version_after == Px4Lite_GetStatusVersion()))
        {
            ...
            out->blocking_fault_mask =
                health.blocking_fault_mask;
            out->warning_fault_mask =
                health.warning_fault_mask;
            out->not_ready_mask =
                health.not_ready_mask;
            out->status_version = version_after;
            out->system_ready =
                ((health.blocking_fault_mask == 0U) &&
                 (health.not_ready_mask == 0U))
                    ? 1U
                    : 0U;
    ...
    uint32_t now_ms)
{
    ...
    memset(out, 0, sizeof(*out));
    if (Px4Lite_CopyAlarmSnapshot(&source) != PX4LITE_OK)
    {
        return PX4LITE_NOT_READY;
    }
    if (Px4Lite_IsFresh(&source.header,
