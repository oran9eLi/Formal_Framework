...
static uint8_t s_subscriptions[BUSINESS_TOPIC_COUNT][BUSINESS_SUBSCRIBER_COUNT];
static Business_TopicStats_t s_topic_stats[BUSINESS_TOPIC_COUNT];
static Business_SubscriberStats_t s_subscriber_stats[BUSINESS_TOPIC_COUNT][BUSINESS_SUBSCRIBER_COUNT];
...
{
  return ((uint32_t)subscriber < (uint32_t)BUSINESS_SUBSCRIBER_COUNT) ? 1U : 0U;
}
...
{
  if ((event == 0) || (Business_IsValidTopic(event->topic) == 0U) || (event->data_len > BUSINESS_EVENT_PAYLOAD_MAX)) { return 0U; }

  if ((event->payload_type == BUSINESS_PAYLOAD_NONE) && (event->data_len != 0U)) { return 0U; }
...
{
  if (report != 0) { (void)memset(report, 0, sizeof(*report)); }
}

/**
 * @brief 在发布报告中标记一个订阅者被本次发布命中。
 */
static void Business_ReportSubscribed(Business_PublishReport_t *report, uint32_t subscriber)
{
  if (report != 0) { report->subscribed_mask |= (1UL << subscriber); }
}

/**
 * @brief 在发布报告中记录一个订阅者投递成功。
 */
static void Business_ReportDelivered(Business_PublishReport_t *report, uint32_t subscriber)
{
  if (report != 0) {
 ...
 */
static void Business_ReportFailed(Business_PublishReport_t *report, uint32_t subscriber)
{
  if (report != 0) {
 ...
 */
static Business_PublishResult_t Business_FinishPublish(Business_Topic_t topic, uint32_t subscriber_count, uint32_t failed_count)
{
 ...
 */
static Business_PublishResult_t Business_FinishPublishFromISR(Business_Topic_t topic, uint32_t subscriber_count, uint32_t failed_count)
{
  Business_PublishResult_t result;
...
BaseType_t Business_EventBusInit(void)
{
  ...
  if (s_initialized != 0U) { return pdPASS; }
  ...
  for (i = 0U; i < (uint32_t)BUSINESS_SUBSCRIBER_COUNT; i++) {
    s_queues[i] = xQueueCreate(BUSINESS_EVENT_QUEUE_LENGTH, sizeof(Business_Event_t));
    if (s_queues[i] == 0) {
      while (i > 0U) {
        i--;
...
{
  if ((Business_IsValidTopic(topic) == 0U) || (Business_IsValidSubscriber(subscriber) == 0U) || (s_initialized == 0U) || (s_queues[subscriber] == 0)) { return pdFAIL; }
...
Business_PublishResult_t Business_EventBusPublish(const Business_Event_t *event, Business_PublishReport_t *report)
{
  ...
  if (s_initialized == 0U) { return BUSINESS_PUBLISH_NOT_READY; }
  if (Business_IsValidEvent(event) == 0U) { return BUSINESS_PUBLISH_INVALID; }

  for (i = 0U; i < (uint32_t)BUSINESS_SUBSCRIBER_COUNT; i++) {
    if (s_subscriptions[event->topic][i] == 0U) { continue; }

    subscriber_count++;
    Business_ReportSubscribed(report, i);

    /* 每个订阅者只尝试一次；某个队列满不会阻塞后续订阅者。 */
    if ((s_queues[i] != 0) && (xQueueSend(s_queues[i], event, 0U) == pdPASS)) {
      taskENTER_CRITICAL();
  ...
  return Business_FinishPublish(event->topic, subscriber_count, failed_count);
}
...
Business_PublishResult_t Business_EventBusPublishFromISR(const Business_Event_t *event, Business_PublishReport_t *report, BaseType_t *higher_priority_task_woken)
{
  ...
  for (i = 0U; i < (uint32_t)BUSINESS_SUBSCRIBER_COUNT; i++) {
    ...
    if ((s_queues[i] != 0) && (xQueueSendFromISR(s_queues[i], event, &task_woken) == pdPASS)) {
      interrupt_mask = portSET_INTERRUPT_MASK_FROM_ISR();
      s_subscriber_stats[event->topic][i].delivered_count++;
      portCLEAR_INTERRUPT_MASK_FROM_ISR(interrupt_mask);
      Business_ReportDelivered(report, i);
      if (task_woken != pdFALSE) { any_task_woken = pdTRUE; }
    } else {
      interrupt_mask = portSET_INTERRUPT_MASK_FROM_ISR();
      s_subscriber_stats[event->topic][i].drop_count++;
      portCLEAR_INTERRUPT_MASK_FROM_ISR(interrupt_mask);
  ...
  if ((higher_priority_task_woken != 0) && (any_task_woken != pdFALSE)) { *higher_priority_task_woken = pdTRUE; }

  return Business_FinishPublishFromISR(event->topic, subscriber_count, failed_count);
}
...
{
  if ((Business_IsValidSubscriber(subscriber) == 0U) || (event == 0) || (s_queues[subscriber] == 0)) { return pdFAIL; }
...
{
  if ((Business_IsValidTopic(topic) == 0U) || (stats == 0)) { return pdFAIL; }
...
{
  if ((Business_IsValidTopic(topic) == 0U) || (Business_IsValidSubscriber(subscriber) == 0U) || (stats == 0)) { return pdFAIL; }
