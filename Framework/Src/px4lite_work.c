 ...
 */
static uint8_t Px4Lite_WorkTimeReached(uint32_t now_ms, uint32_t deadline_ms)
{
...
{
  if ((item == 0) || (name == 0) || (period_ms == 0U) || (run == 0)) { return PX4LITE_INVALID_PARAM; }
...
Px4Lite_Result_t Px4Lite_WorkRunDue(Px4Lite_WorkItem_t *item, uint32_t now_ms)
{
  ...
  if ((item == 0) || (item->initialized == 0U) || (item->run == 0)) { return PX4LITE_INVALID_PARAM; }

  if (Px4Lite_WorkTimeReached(now_ms, item->next_run_ms) == 0U) { return PX4LITE_IDLE; }

  while (Px4Lite_WorkTimeReached(now_ms, item->next_run_ms + item->period_ms) != 0U) {
  ...
  item->next_run_ms += item->period_ms;
  item->run_count++;
  if (elapsed_us > item->max_execution_us) { item->max_execution_us = elapsed_us; }
  return PX4LITE_OK;
}
