    ...
    const Px4Lite_SensorGnss_t *gnss)
{
    ...
    if (gnss->fix_type == 0U)
    {
        return ((gnss->gps_visible + gnss->bds_visible) != 0U)
                   ? 20U
                   : 5U;
    }

    quality = 100U;
    if (gnss->satellites_used < 8U)
    {
        quality = (uint8_t)(quality - 20U);
    }
...
{
    if (state == PX4LITE_STATE_DEGRADED)
    {
        return (uint8_t)PX4LITE_SEVERITY_WARNING;
    }
    if (state == PX4LITE_STATE_OFFLINE)
    {
        return (uint8_t)PX4LITE_SEVERITY_ERROR;
    }
    if (state == PX4LITE_STATE_FAILED)
    {
        return (uint8_t)PX4LITE_SEVERITY_CRITICAL;
    }
                              ...
                              uint32_t now_ms)
{
    ...
    if ((uint32_t)id >= (uint32_t)PX4LITE_MODULE_COUNT)
    {
        return;
    }
    ...
    if (item->state != state)
    {
        if (((item->state == PX4LITE_STATE_DEGRADED) ||
             (item->state == PX4LITE_STATE_OFFLINE) ||
             (item->state == PX4LITE_STATE_FAILED)) &&
            (state == PX4LITE_STATE_ONLINE))
        {
            if (item->recovery_count < 255U)
            {
                item->recovery_count++;
            }
        }
                                        ...
                                        uint32_t now_ms)
{
    ...
    if (s_status[id].consecutive_errors < 65535U)
    {
        s_status[id].consecutive_errors++;
    }
    s_status_version++;
    taskEXIT_CRITICAL();
}
...
Px4Lite_Result_t Px4Lite_ModulesInit(void)
{
    ...
    for (i = 0U; i < (uint32_t)PX4LITE_MODULE_COUNT; ++i)
    {
        ...
        s_status[i].state =
            (Px4Lite_IsModuleEnabled((Px4Lite_ModuleId_t)i) != 0U)
                                     ...
                                     uint32_t now_ms)
{
    Px4Lite_SetStatus(
        id,
        (init_result == PX4LITE_OK) ? PX4LITE_STATE_STARTING
                                    : PX4LITE_STATE_FAILED,
        (init_result == PX4LITE_OK) ? 0U
                                    : PX4LITE_FAULT_SENSOR_INIT,
        now_ms);
}
...
Px4Lite_Result_t Px4Lite_GnssModuleInit(void)
{
    ...
    Px4Lite_PublishInitState(PX4LITE_MODULE_GNSS,
                             Px4Lite_GnssInit(),
...
Px4Lite_Result_t Px4Lite_ImuModuleInit(void)
{
    ...
    Px4Lite_PublishInitState(PX4LITE_MODULE_IMU,
                             Px4Lite_ImuInit(),
                             Px4Lite_PlatformGetMs());
#endif
    return PX4LITE_OK;
...
Px4Lite_Result_t Px4Lite_BaroModuleInit(void)
{
    ...
    Px4Lite_PublishInitState(PX4LITE_MODULE_BARO,
                             Px4Lite_BaroInit(),
                             Px4Lite_PlatformGetMs());
#endif
    return PX4LITE_OK;
...
Px4Lite_Result_t Px4Lite_BatteryModuleInit(void)
{
    ...
    Px4Lite_PublishInitState(PX4LITE_MODULE_BATTERY,
                             Px4Lite_BatteryInit(),
                             Px4Lite_PlatformGetMs());
#endif
    return PX4LITE_OK;
...
Px4Lite_Result_t Px4Lite_AlarmModuleInit(void)
{
    ...
    Px4Lite_SetStatus(
        ...
        (result == PX4LITE_OK)
            ? PX4LITE_FAULT_NONE
            : PX4LITE_FAULT_SYSTEM_SELF_CHECK,
        now_ms);
    return result;
#else
    return PX4LITE_OK;
...
void Px4Lite_SensorWorkRun(uint32_t now_ms)
{
    ...
    {
        gnss.header.sample_time_ms =
            (gnss.header.sample_time_ms != 0U)
                ? gnss.header.sample_time_ms
                : now_ms;
        ...
        if (gnss.fix_type == 0U)
        {
            gnss.header.flags |= PX4LITE_DATA_DEGRADED;
        }
        Px4Lite_PublishGnss(&gnss);

        taskENTER_CRITICAL();
        s_status[PX4LITE_MODULE_GNSS].last_rx_ms =
            gnss.header.sample_time_ms;
        s_status[PX4LITE_MODULE_GNSS].consecutive_errors = 0U;
        if (s_status[PX4LITE_MODULE_GNSS].consecutive_valid < 65535U)
        {
            s_status[PX4LITE_MODULE_GNSS].consecutive_valid++;
        }
        if (gnss.fix_type != 0U)
        {
            s_status[PX4LITE_MODULE_GNSS].last_valid_ms =
                gnss.header.sample_time_ms;
        }
        s_status_version++;
        taskEXIT_CRITICAL();

        Px4Lite_SetStatus(PX4LITE_MODULE_GNSS,
                          (gnss.fix_type != 0U)
                              ? PX4LITE_STATE_ONLINE
                              : PX4LITE_STATE_DEGRADED,
                          (gnss.fix_type != 0U)
                              ? 0U
                              : PX4LITE_FAULT_SENSOR_NO_FIX,
                          now_ms);
    }
...
#if PX4LITE_ENABLE_IMU
    if ((s_last_imu_work_ms == 0U) ||
        ((uint32_t)(now_ms - s_last_imu_work_ms) >=
         PX4LITE_IMU_WORK_PERIOD_MS))
    {
        ...
        if (result == PX4LITE_OK)
        {
            ...
            taskENTER_CRITICAL();
            s_status[PX4LITE_MODULE_IMU].last_rx_ms =
                imu.header.sample_time_ms;
            s_status[PX4LITE_MODULE_IMU].last_valid_ms =
                imu.header.sample_time_ms;
            s_status[PX4LITE_MODULE_IMU].consecutive_errors = 0U;
            if (s_status[PX4LITE_MODULE_IMU].consecutive_valid < 65535U)
            {
                s_status[PX4LITE_MODULE_IMU].consecutive_valid++;
            }
            s_status_version++;
            taskEXIT_CRITICAL();

            Px4Lite_SetStatus(PX4LITE_MODULE_IMU,
                              PX4LITE_STATE_ONLINE,
                              PX4LITE_FAULT_NONE,
                              now_ms);
        }
...
#if PX4LITE_ENABLE_BARO
    if ((s_last_baro_work_ms == 0U) ||
        ((uint32_t)(now_ms - s_last_baro_work_ms) >=
         PX4LITE_BARO_WORK_PERIOD_MS))
    {
        ...
        if (result == PX4LITE_OK)
        {
            baro.header.sample_time_ms =
                (baro.header.sample_time_ms != 0U)
                    ? baro.header.sample_time_ms
                    : now_ms;
            ...
            taskENTER_CRITICAL();
            s_status[PX4LITE_MODULE_BARO].last_rx_ms =
                baro.header.sample_time_ms;
            s_status[PX4LITE_MODULE_BARO].last_valid_ms =
                baro.header.sample_time_ms;
            s_status[PX4LITE_MODULE_BARO].consecutive_errors = 0U;
            if (s_status[PX4LITE_MODULE_BARO].consecutive_valid < 65535U)
            {
                s_status[PX4LITE_MODULE_BARO].consecutive_valid++;
            }
            s_status_version++;
            taskEXIT_CRITICAL();

            Px4Lite_SetStatus(PX4LITE_MODULE_BARO,
                              PX4LITE_STATE_ONLINE,
                              PX4LITE_FAULT_NONE,
                              now_ms);
        }
...
#if PX4LITE_ENABLE_BATTERY
    if ((s_last_battery_work_ms == 0U) ||
        ((uint32_t)(now_ms - s_last_battery_work_ms) >=
         PX4LITE_BATTERY_WORK_PERIOD_MS))
    {
        ...
        if (result == PX4LITE_OK)
        {
            battery.header.sample_time_ms =
                (battery.header.sample_time_ms != 0U)
                    ? battery.header.sample_time_ms
                    : now_ms;
            battery.header.publish_time_ms = now_ms;
            battery.header.sequence = ++s_battery_sequence;
            battery.header.device_id =
                (uint16_t)PX4LITE_MODULE_BATTERY;
            battery.header.valid = 1U;
            battery.header.quality =
                (battery.low_voltage != 0U) ? 50U : 100U;
            battery.header.flags = PX4LITE_DATA_VALID;
            if (battery.low_voltage != 0U)
            {
                battery.header.flags |= PX4LITE_DATA_DEGRADED;
            }
            Px4Lite_PublishBattery(&battery);

            taskENTER_CRITICAL();
            s_status[PX4LITE_MODULE_BATTERY].last_rx_ms =
                battery.header.sample_time_ms;
            s_status[PX4LITE_MODULE_BATTERY].last_valid_ms =
                battery.header.sample_time_ms;
            s_status[PX4LITE_MODULE_BATTERY].consecutive_errors = 0U;
            if (s_status[PX4LITE_MODULE_BATTERY].consecutive_valid < 65535U)
            {
                s_status[PX4LITE_MODULE_BATTERY].consecutive_valid++;
            }
            s_status_version++;
            taskEXIT_CRITICAL();

            Px4Lite_SetStatus(PX4LITE_MODULE_BATTERY,
...
#if !PX4LITE_ENABLE_GNSS && !PX4LITE_ENABLE_IMU && \
    !PX4LITE_ENABLE_BARO && !PX4LITE_ENABLE_BATTERY
    (void)now_ms;
#endif
}
...
Px4Lite_Result_t Px4Lite_EstimatorInit(void)
{
    ...
    Px4Lite_SetStatus(PX4LITE_MODULE_ESTIMATOR,
                      PX4LITE_STATE_ONLINE,
                      PX4LITE_FAULT_NONE,
                      Px4Lite_PlatformGetMs());
    return PX4LITE_OK;
...
void Px4Lite_EstimatorRun(uint32_t now_ms)
{
    ...
    if ((Px4Lite_CopyGnss(&gnss) == PX4LITE_OK) &&
        (Px4Lite_IsFresh(&gnss.header, now_ms,
                         PX4LITE_GNSS_MAX_AGE_MS) != 0U))
    {
        ...
        if (gnss.fix_type != 0U)
        {
            navigation.valid_mask =
                PX4LITE_NAV_VALID_POSITION |
                PX4LITE_NAV_VALID_ALTITUDE;
        }
        else
        {
    ...
    while ((imu_pop_count < 8U) &&
           (Px4Lite_PopImu(&imu) == PX4LITE_OK))
    {
        imu_pop_count++;
        if ((imu.header.valid != 0U) &&
            (Px4Lite_IsFresh(&imu.header, now_ms,
                             PX4LITE_IMU_MAX_AGE_MS) != 0U) &&
            (Px4Lite_AttitudeUpdate(&s_attitude_state,
                                    &imu,
                                    &roll_deg100,
                                    &pitch_deg100,
                                    &yaw_deg100) == PX4LITE_OK))
        {
            attitude_updated = 1U;
            roll_rate_dps100 = imu.gyro_mdps[0] / 10;
            pitch_rate_dps100 = imu.gyro_mdps[1] / 10;
            yaw_rate_dps100 = imu.gyro_mdps[2] / 10;
            navigation.header.sample_time_ms =
                imu.header.sample_time_ms;
        }
    }
    ...
    if (publish_navigation == 0U)
    {
        return;
    }

    if (navigation.header.sample_time_ms == 0U)
    {
        navigation.header.sample_time_ms = now_ms;
    }
...
void Px4Lite_HealthRun(uint32_t now_ms)
{
    ...
    lora_last_valid_ms =
        s_status[PX4LITE_MODULE_LORA].last_valid_ms;
    lora_state = s_status[PX4LITE_MODULE_LORA].state;
    display_last_valid_ms =
        s_status[PX4LITE_MODULE_DISPLAY].last_valid_ms;
    display_state = s_status[PX4LITE_MODULE_DISPLAY].state;
    taskEXIT_CRITICAL();

#if PX4LITE_ENABLE_GNSS
    if ((gnss_state != PX4LITE_STATE_FAILED) &&
        (Px4Lite_ElapsedMs(now_ms, last_rx_ms) >
         PX4LITE_GNSS_OFFLINE_MS))
    {
        if ((last_rx_ms != 0U) ||
            (Px4Lite_ElapsedMs(now_ms, s_start_ms) >
             PX4LITE_GNSS_STARTUP_GRACE_MS))
        {
            Px4Lite_SetStatus(PX4LITE_MODULE_GNSS,
                              PX4LITE_STATE_OFFLINE,
                              PX4LITE_FAULT_SENSOR_OFFLINE,
                              now_ms);
        }
    }
#else
...
#if PX4LITE_ENABLE_IMU
    if ((imu_state != PX4LITE_STATE_FAILED) &&
        (Px4Lite_ElapsedMs(now_ms, imu_last_rx_ms) >
         PX4LITE_IMU_OFFLINE_MS))
    {
        if ((imu_last_rx_ms != 0U) ||
            (Px4Lite_ElapsedMs(now_ms, s_start_ms) >
             PX4LITE_IMU_STARTUP_GRACE_MS))
        {
            Px4Lite_SetStatus(PX4LITE_MODULE_IMU,
                              PX4LITE_STATE_OFFLINE,
                              PX4LITE_FAULT_SENSOR_OFFLINE,
...
#if PX4LITE_ENABLE_BARO
    if ((baro_state != PX4LITE_STATE_FAILED) &&
        (Px4Lite_ElapsedMs(now_ms, baro_last_rx_ms) >
         PX4LITE_BARO_OFFLINE_MS))
    {
        if ((baro_last_rx_ms != 0U) ||
            (Px4Lite_ElapsedMs(now_ms, s_start_ms) >
             PX4LITE_BARO_STARTUP_GRACE_MS))
        {
            Px4Lite_SetStatus(PX4LITE_MODULE_BARO,
                              PX4LITE_STATE_OFFLINE,
                              PX4LITE_FAULT_SENSOR_OFFLINE,
...
#if PX4LITE_ENABLE_BATTERY
    if ((battery_state != PX4LITE_STATE_FAILED) &&
        (Px4Lite_ElapsedMs(now_ms, battery_last_rx_ms) >
         PX4LITE_BATTERY_OFFLINE_MS))
    {
        if ((battery_last_rx_ms != 0U) ||
            (Px4Lite_ElapsedMs(now_ms, s_start_ms) >
             PX4LITE_BATTERY_STARTUP_GRACE_MS))
        {
            Px4Lite_SetStatus(PX4LITE_MODULE_BATTERY,
                              PX4LITE_STATE_OFFLINE,
                              PX4LITE_FAULT_SENSOR_OFFLINE,
     ...
     */
    if ((lora_state != PX4LITE_STATE_FAILED) &&
        (Px4Lite_ElapsedMs(now_ms, lora_last_valid_ms) >
         PX4LITE_LORA_OFFLINE_MS))
    {
        if ((lora_last_valid_ms != 0U) ||
            (Px4Lite_ElapsedMs(now_ms, s_start_ms) >
             PX4LITE_LORA_STARTUP_GRACE_MS))
        {
            Px4Lite_SetStatus(PX4LITE_MODULE_LORA,
                              PX4LITE_STATE_OFFLINE,
                              PX4LITE_FAULT_COMM_OFFLINE,
                              now_ms);
        }
    }
#else
    ...
    if ((display_state != PX4LITE_STATE_FAILED) &&
        (Px4Lite_ElapsedMs(now_ms, display_last_valid_ms) >
         PX4LITE_DISPLAY_OFFLINE_MS))
    {
        if ((display_last_valid_ms != 0U) ||
            (Px4Lite_ElapsedMs(now_ms, s_start_ms) >
             PX4LITE_DISPLAY_STARTUP_GRACE_MS))
        {
            Px4Lite_SetStatus(PX4LITE_MODULE_DISPLAY,
                              PX4LITE_STATE_OFFLINE,
    ...
    if (Px4Lite_CopyModuleStatuses(
            status,
            PX4LITE_MODULE_COUNT,
            &status_version) != PX4LITE_OK)
    {
        return;
    }
    ...
    for (i = 0U; i < (uint32_t)PX4LITE_MODULE_COUNT; ++i)
    {
        ...
        else if ((status[i].state == PX4LITE_STATE_DEGRADED) ||
                 (status[i].state == PX4LITE_STATE_OFFLINE))
        {
            health.warning_fault_mask |= (1UL << i);
        }
        else if ((status[i].state == PX4LITE_STATE_UNINITIALIZED) ||
                 (status[i].state == PX4LITE_STATE_STARTING))
        {
    ...
    Px4Lite_AlarmUpdateFromStatuses(
        status,
        (uint16_t)PX4LITE_MODULE_COUNT,
        now_ms);
...
{
    if ((status == 0) ||
        ((uint32_t)module_id >= (uint32_t)PX4LITE_MODULE_COUNT))
    {
...
void Px4Lite_CommWorkRun(uint32_t now_ms)
{
    ...
    if (result == PX4LITE_OK)
    {
        ...
        if ((tx_result != PX4LITE_OK) &&
            (tx_result != PX4LITE_IDLE) &&
            (tx_result != PX4LITE_NOT_READY) &&
            (tx_result != PX4LITE_STALE) &&
            (tx_result != PX4LITE_BUSY)) {
            result = tx_result;
        }
    }
    ...
    if ((last_valid_ms == 0U) ||
        ((info.last_tx_ms != 0U) &&
         ((int32_t)(info.last_tx_ms -
                    last_valid_ms) > 0)))
    {
        last_valid_ms = info.last_tx_ms;
    }

    taskENTER_CRITICAL();
    s_status[PX4LITE_MODULE_LORA].last_rx_ms = info.last_rx_ms;
    s_status[PX4LITE_MODULE_LORA].last_valid_ms = last_valid_ms;
    s_status[PX4LITE_MODULE_LORA].error_count =
        info.parse_error_count + info.send_error_count;
    s_status[PX4LITE_MODULE_LORA].drop_count =
        info.rx_drop_count + info.rx_overflow_count;
    taskEXIT_CRITICAL();
...
void Px4Lite_GetCommDebugInfo(Px4Lite_CommDebugInfo_t *out)
{
    ...
    out->mav_gnss_detail_count =
        stats.gnss_detail_count;
    ...
    out->mav_battery_status_count =
        stats.battery_status_count;
    out->mav_scaled_pressure_count =
        stats.scaled_pressure_count;
