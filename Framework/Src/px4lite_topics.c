...
Px4Lite_Result_t Px4Lite_TopicsInit(void)
{
  ...
  s_alarm_queue = xQueueCreate(PX4LITE_ALARM_QUEUE_LENGTH, sizeof(Px4Lite_AlarmEvent_t));
  if (s_alarm_queue == 0) { return PX4LITE_IO_ERROR; }
#endif

#if PX4LITE_ENABLE_COMMAND
  s_command_queue     = xQueueCreate(PX4LITE_COMMAND_QUEUE_LENGTH, sizeof(Px4Lite_Command_t));
  s_command_ack_queue = xQueueCreate(PX4LITE_COMMAND_QUEUE_LENGTH, sizeof(Px4Lite_CommandAck_t));
  if ((s_command_queue == 0) || (s_command_ack_queue == 0)) { return PX4LITE_IO_ERROR; }
#endif
...
Px4Lite_Result_t Px4Lite_PushImu(const Px4Lite_SensorImu_t *data)
{
  ...
  if (s_imu_fifo.count >= PX4LITE_IMU_FIFO_CAPACITY) {
    s_imu_fifo.tail = (uint16_t)((s_imu_fifo.tail + 1U) % PX4LITE_IMU_FIFO_CAPACITY);
    s_imu_fifo.count--;
  ...
  s_imu_fifo.head                     = (uint16_t)((s_imu_fifo.head + 1U) % PX4LITE_IMU_FIFO_CAPACITY);
  s_imu_fifo.count++;
  s_imu_fifo.stats.count = s_imu_fifo.count;
  s_imu_fifo.stats.push_count++;

  if (s_imu_fifo.count > s_imu_fifo.stats.peak) { s_imu_fifo.stats.peak = s_imu_fifo.count; }
...
{
  if (event == 0) { return PX4LITE_INVALID_PARAM; }
  if (s_alarm_queue == 0) { return PX4LITE_NOT_READY; }

  return (xQueueSend(s_alarm_queue, event, 0U) == pdPASS) ? PX4LITE_OK : PX4LITE_OVERFLOW;
}
...
Px4Lite_Result_t Px4Lite_TakeAlarm(Px4Lite_AlarmEvent_t *event)
{
  ...
  return (xQueueReceive(s_alarm_queue, event, 0U) == pdPASS) ? PX4LITE_OK : PX4LITE_NOT_READY;
}
...
{
  if (command == 0) { return PX4LITE_INVALID_PARAM; }
  if (s_command_queue == 0) { return PX4LITE_NOT_READY; }

  return (xQueueSend(s_command_queue, command, 0U) == pdPASS) ? PX4LITE_OK : PX4LITE_OVERFLOW;
}
...
Px4Lite_Result_t Px4Lite_TakeCommand(Px4Lite_Command_t *command)
{
  ...
  return (xQueueReceive(s_command_queue, command, 0U) == pdPASS) ? PX4LITE_OK : PX4LITE_NOT_READY;
}
...
{
  if (ack == 0) { return PX4LITE_INVALID_PARAM; }
  if (s_command_ack_queue == 0) { return PX4LITE_NOT_READY; }

  return (xQueueSend(s_command_ack_queue, ack, 0U) == pdPASS) ? PX4LITE_OK : PX4LITE_OVERFLOW;
}
...
Px4Lite_Result_t Px4Lite_TakeCommandAck(Px4Lite_CommandAck_t *ack)
{
  ...
  return (xQueueReceive(s_command_ack_queue, ack, 0U) == pdPASS) ? PX4LITE_OK : PX4LITE_NOT_READY;
}
