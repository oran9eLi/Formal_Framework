 ...
 * @brief 返回指定年月的天数。
 */
static uint8_t Px4Lite_TimeDaysInMonth(uint16_t year, uint8_t month)
{
  static const uint8_t days[12] = {31U, 28U, 31U, 30U, 31U, 30U, 31U, 31U, 30U, 31U, 30U, 31U};

  if ((month < 1U) || (month > 12U)) { return 31U; }
  if (month == 2U) { return (Px4Lite_TimeIsLeapYear(year) != 0U) ? 29U : 28U; }
 ...
 */
static uint8_t Px4Lite_TimeIsDateTimeValid(const Px4Lite_UtcDateTime_t *date_time)
{
  if (date_time == 0) { return 0U; }
...
{
  date_time->day++;
  if (date_time->day > Px4Lite_TimeDaysInMonth(date_time->year, date_time->month)) {
    date_time->day = 1U;
    date_time->month++;
    if (date_time->month > 12U) {
      date_time->month = 1U;
      if (date_time->year < 2099U) { date_time->year++; }
    }
  }
}
 ...
 */
static void Px4Lite_TimeAddSeconds(Px4Lite_UtcDateTime_t *date_time, uint32_t add_seconds)
{
  uint32_t seconds;

  seconds = ((uint32_t)date_time->hours * 3600UL) + ((uint32_t)date_time->minutes * 60UL) + date_time->seconds + add_seconds;
 ...
 */
static uint32_t Px4Lite_TimeEncodeDateYmd(const Px4Lite_UtcDateTime_t *date_time)
{
  return ((uint32_t)date_time->year * 10000UL) + ((uint32_t)date_time->month * 100UL) + date_time->day;
}

/**
 * @brief 将时间编码为 HHMMSS。
 */
static uint32_t Px4Lite_TimeEncodeTimeHhmmss(const Px4Lite_UtcDateTime_t *date_time)
{
  return ((uint32_t)date_time->hours * 10000UL) + ((uint32_t)date_time->minutes * 100UL) + date_time->seconds;
}

/**
 * @brief 将 GNSS yymmdd 和当日秒数转换为 UTC 日期时间。
 */
static uint8_t Px4Lite_TimeFromGnss(uint32_t utc_date, uint32_t utc_sec, Px4Lite_UtcDateTime_t *out)
{
  uint32_t yy;
  uint32_t mm;
  uint32_t dd;
 ...
 */
static void Px4Lite_TimeFillSnapshot(Px4Lite_TimeSnapshot_t *snapshot, const Px4Lite_UtcDateTime_t *utc, Px4Lite_TimeSource_t source, Px4Lite_TimeSyncState_t sync_state, uint32_t now_ms)
{
  ...
  snapshot->header.quality         = (source == PX4LITE_TIME_SOURCE_GNSS) ? 100U : 70U;
  ...
  snapshot->sync_age_s             = (s_last_gnss_sync_ms != 0U) ? (Px4Lite_ElapsedMs(now_ms, s_last_gnss_sync_ms) / 1000UL) : 0U;
 ...
 * @brief 发布新的统一时间快照。
 */
static void Px4Lite_TimePublish(const Px4Lite_UtcDateTime_t *utc, Px4Lite_TimeSource_t source, Px4Lite_TimeSyncState_t sync_state, uint32_t now_ms)
{
 ...
 */
static uint8_t Px4Lite_TimeCopyValidGnssTime(uint32_t now_ms, Px4Lite_UtcDateTime_t *out)
{
...
void Px4Lite_TimeRun(uint32_t now_ms)
{
  ...
  rtc_state = ((s_last_gnss_sync_ms != 0U) && (Px4Lite_ElapsedMs(now_ms, s_last_gnss_sync_ms) > PX4LITE_TIME_STALE_MS)) ? PX4LITE_TIME_SYNC_STALE : PX4LITE_TIME_SYNC_RTC_VALID;
  Px4Lite_TimePublish(&rtc_time, PX4LITE_TIME_SOURCE_RTC, rtc_state, now_ms);
}
...
Px4Lite_Result_t Px4Lite_CopyTime(Px4Lite_TimeSnapshot_t *out)
{
  ...
  taskENTER_CRITICAL();
  ready = s_time_ready;
  if (ready != 0U) { *out = s_time_snapshot; }
  taskEXIT_CRITICAL();
