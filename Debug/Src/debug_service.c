...
static void DebugService_ReportGnss(uint32_t now_ms)
{
    ...
    if ((Px4Lite_CopyGnss(&gnss) == PX4LITE_OK) &&
        (Px4Lite_GetModuleStatus(PX4LITE_MODULE_GNSS,
                                 &status) == PX4LITE_OK))
    {
        report_ms = Px4Lite_PlatformGetMs();
        DBG_PRINT(
            "GNSS: state=%u fix=%u sats=%u gps=%u/%u bds=%u/%u "
            "lat=%ld lon=%ld age=%lu",
            (unsigned int)status.state,
            (unsigned int)gnss.fix_type,
            (unsigned int)gnss.satellites_used,
            (unsigned int)gnss.gps_used,
            (unsigned int)gnss.gps_visible,
            (unsigned int)gnss.bds_used,
            (unsigned int)gnss.bds_visible,
            (long)gnss.latitude_e7,
            (long)gnss.longitude_e7,
            (unsigned long)Px4Lite_ElapsedMs(report_ms,
                                             status.last_rx_ms));
    }
    else
    {
    ...
    if (status_rc == PX4LITE_OK)
    {
        ...
        imu_fresh =
            ((status.state == PX4LITE_STATE_ONLINE) &&
             (imu_age_ms <= PX4LITE_IMU_MAX_AGE_MS))
                ? 1U
                : 0U;
        if ((nav_rc == PX4LITE_OK) &&
            ((nav.valid_mask & PX4LITE_NAV_VALID_ATTITUDE) != 0U))
        {
            DBG_PRINT(
                "IMU: state=%u err=%lu age=%lu fresh=%u "
                "roll_cdeg=%ld pitch_cdeg=%ld yaw_cdeg=%ld "
                "rate_cdps=%ld,%ld,%ld",
                (unsigned int)status.state,
                (unsigned long)status.error_count,
                (unsigned long)imu_age_ms,
                (unsigned int)imu_fresh,
                (long)nav.roll_deg100,
                (long)nav.pitch_deg100,
                (long)nav.yaw_deg100,
                (long)nav.roll_rate_dps100,
                (long)nav.pitch_rate_dps100,
                (long)nav.yaw_rate_dps100);
        }
        else
        {
            DBG_PRINT(
                "IMU: state=%u fault=%u err=%lu age=%lu fresh=%u "
                "no valid attitude rc=%d mask=0x%08lX",
                (unsigned int)status.state,
                (unsigned int)status.fault_code,
                (unsigned long)status.error_count,
                (unsigned long)imu_age_ms,
                (unsigned int)imu_fresh,
                (int)nav_rc,
                (nav_rc == PX4LITE_OK)
                    ? (unsigned long)nav.valid_mask
                    : 0UL);
        }
    }
    else
    {
        DBG_PRINT("IMU: framework status unavailable rc=%d",
                  (int)status_rc);
    }
}
#endif
...
static void DebugService_ReportBaro(uint32_t now_ms)
{
    ...
    if ((Px4Lite_CopyBaro(&baro) == PX4LITE_OK) &&
        (Px4Lite_GetModuleStatus(PX4LITE_MODULE_BARO,
                                 &status) == PX4LITE_OK))
    {
        report_ms = Px4Lite_PlatformGetMs();
        DBG_PRINT(
            "BARO: state=%u pressure_pa=%ld temp_cC=%d alt_mm=%ld "
            "vs_cms=%ld age=%lu",
            (unsigned int)status.state,
            (long)baro.pressure_pa,
            (int)(baro.temperature_c * 100.0f),
            (long)baro.pressure_altitude_mm,
            (long)baro.vertical_speed_cms,
            (unsigned long)Px4Lite_ElapsedMs(report_ms,
                                             status.last_rx_ms));
    }
    else
...
static void DebugService_ReportPower(uint32_t now_ms)
{
    ...
    if ((Px4Lite_CopyBattery(&battery) == PX4LITE_OK) &&
        (Px4Lite_GetModuleStatus(PX4LITE_MODULE_BATTERY,
                                 &status) == PX4LITE_OK))
    {
        report_ms = Px4Lite_PlatformGetMs();
        DBG_PRINT(
            "POWER: state=%u voltage_mv=%lu current_ma=%ld "
            "percent=%u low=%u age=%lu",
            (unsigned int)status.state,
            (unsigned long)battery.voltage_mv,
            (long)battery.current_ma,
            (unsigned int)battery.percent,
            (unsigned int)battery.low_voltage,
            (unsigned long)Px4Lite_ElapsedMs(report_ms,
                                             status.last_rx_ms));
    }
    else
...
static uint32_t DebugService_RatePermille(uint32_t error_count,
...
static void DebugService_ReportLoRa(uint32_t now_ms)
{
    ...
    rx_error_total = info.crc_error_count +
                     info.parse_error_count +
                     info.rx_drop_count +
                     info.rx_overflow_count;
    tx_error_total = info.tx_busy_count +
                     info.send_error_count +
                     info.mav_error_count;
    rx_loss_permille =
        DebugService_RatePermille(rx_error_total, info.rx_frame_count);
    tx_fail_permille =
        DebugService_RatePermille(tx_error_total, info.tx_frame_count);

    report_ms = Px4Lite_PlatformGetMs();
    DBG_PRINT(
        "LORA: state=%u rx=%lu tx=%lu rx_loss=%lu.%lu%% "
        ...
        (unsigned int)status.state,
        (unsigned long)info.rx_frame_count,
        (unsigned long)info.tx_frame_count,
        (unsigned long)(rx_loss_permille / 10U),
        (unsigned long)(rx_loss_permille % 10U),
        (unsigned long)(tx_fail_permille / 10U),
        (unsigned long)(tx_fail_permille % 10U),
        (unsigned long)info.tx_busy_count,
        (unsigned long)info.crc_error_count,
        (unsigned long)info.parse_error_count,
        (unsigned long)info.rx_drop_count,
        (unsigned long)info.rx_overflow_count,
        (unsigned long)info.mav_heartbeat_count,
        (unsigned long)info.mav_gps_raw_count,
        (unsigned long)info.mav_gnss_detail_count,
        (unsigned long)info.mav_attitude_count,
        (unsigned long)info.mav_sys_status_count,
        (unsigned long)info.mav_battery_status_count,
        (unsigned long)info.mav_scaled_pressure_count,
        (unsigned long)info.mav_statustext_count,
        (unsigned long)info.mav_stale_count,
        (unsigned long)info.mav_no_data_count,
        (unsigned long)Px4Lite_ElapsedMs(report_ms, info.last_rx_ms),
        (unsigned long)Px4Lite_ElapsedMs(report_ms, info.last_tx_ms),
...
static void DebugService_ReportAlarm(uint32_t now_ms)
{
    ...
    {
        DBG_PRINT(
            "ALARM: count=%u highest_src=%u fault=0x%04X sev=%u",
            (unsigned int)alarm.active_count,
            (unsigned int)alarm.highest_source_id,
            (unsigned int)alarm.highest_fault_code,
            (unsigned int)alarm.highest_severity);
    }
    else
    {
...
static void DebugService_Task(void *argument)
{
    ...
    for (;;)
    {
...
#if DEBUG_STACK_MONITOR_ENABLE
        if ((uint32_t)(now_ms - last_stack_report_ms) >=
            DEBUG_STACK_REPORT_PERIOD_MS)
        {
...
#if DEBUG_GNSS_MONITOR_ENABLE
        if ((uint32_t)(now_ms - last_gnss_report_ms) >=
            DEBUG_GNSS_REPORT_PERIOD_MS)
        {
...
#if DEBUG_IMU_MONITOR_ENABLE
        if ((uint32_t)(now_ms - last_imu_report_ms) >=
            DEBUG_IMU_REPORT_PERIOD_MS)
        {
...
#if DEBUG_BARO_MONITOR_ENABLE
        if ((uint32_t)(now_ms - last_baro_report_ms) >=
            DEBUG_BARO_REPORT_PERIOD_MS)
        {
...
#if DEBUG_POWER_MONITOR_ENABLE
        if ((uint32_t)(now_ms - last_power_report_ms) >=
            DEBUG_POWER_REPORT_PERIOD_MS)
        {
...
#if DEBUG_LORA_MONITOR_ENABLE
        if ((uint32_t)(now_ms - last_lora_report_ms) >=
            DEBUG_LORA_REPORT_PERIOD_MS)
        {
...
#if DEBUG_ALARM_MONITOR_ENABLE
        if ((uint32_t)(now_ms - last_alarm_report_ms) >=
            DEBUG_ALARM_REPORT_PERIOD_MS)
        {
        ...
        vTaskDelayUntil(&last_wake,
                        pdMS_TO_TICKS(DEBUG_SERVICE_PERIOD_MS));
    }
}
...
BaseType_t DebugService_CreateTask(void)
{
    ...
    if (xTaskCreate(DebugService_Task,
                    "debug",
                    DEBUG_SERVICE_TASK_STACK_WORDS,
                    0,
                    DEBUG_SERVICE_TASK_PRIORITY,
                    &task_handle) != pdPASS)
    {
        return pdFAIL;
    }
