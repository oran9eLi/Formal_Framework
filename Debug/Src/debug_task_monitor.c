...
static DebugTaskMonitor_Record_t
    s_task_records[DEBUG_TASK_MONITOR_CAPACITY];
...
{
#if DEBUG_STACK_MONITOR_ENABLE
    if ((handle == 0) || (name == 0) || (stack_words == 0U) ||
        (s_task_count >= DEBUG_TASK_MONITOR_CAPACITY))
    {
        return pdFAIL;
    }
...
void DebugTaskMonitor_Report(void)
{
    ...
    DBG_PRINT("MEM: heap_cfg=%lu heap_free=%lu heap_min=%lu heap_peak_used=%lu",
              (unsigned long)configTOTAL_HEAP_SIZE,
              (unsigned long)heap_free,
              (unsigned long)heap_min,
              (unsigned long)
                  (configTOTAL_HEAP_SIZE - heap_min));
    ...
    for (index = 0U; index < s_task_count; ++index)
    {
        ...
        free_words = uxTaskGetStackHighWaterMark(
            s_task_records[index].handle);
        used_words =
            (free_words < s_task_records[index].configured_words)
                ? ((uint32_t)s_task_records[index].configured_words -
                   (uint32_t)free_words)
                : 0U;
        warning =
            ((free_words < DEBUG_STACK_WARN_MIN_WORDS) ||
             (((uint32_t)free_words * 4U) <
              s_task_records[index].configured_words))
                ? 1U
                : 0U;

        DBG_PRINT("STACK: task=%s cfg=%uW peak=%luW min_free=%luW/%luB %s",
                  s_task_records[index].name,
                  (unsigned int)
                      s_task_records[index].configured_words,
                  (unsigned long)used_words,
                  (unsigned long)free_words,
                  (unsigned long)
                      ((uint32_t)free_words * sizeof(StackType_t)),
                  (warning != 0U) ? "WARN" : "OK");
    }
#endif
}
