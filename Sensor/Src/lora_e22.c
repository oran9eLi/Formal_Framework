...
Lora_Result_t Lora_E22_Init(void)
{
    ...
    while (BSP_LoRa_IsReady() == 0U) {
        if ((uint32_t)(BSP_Time_GetTickMs() - start_ms) > 500U) {
...
Lora_Result_t Lora_E22_Service(uint32_t now_ms)
{
    ...
    while ((available = BSP_LoRa_GetRxCount()) > 0U) {
        ...
        for (i = 0U; i < received; i++) {
            ...
            if (mavlink_parse_char(MAVLINK_COMM_0, temp[i],
                                   &s_parse_msg,
                                   &s_parse_status) != 0) {
                ...
                s_rx_frame.frame_len = s_parse_msg.len +
                ...
                memcpy(s_rx_frame.data,
                       _MAV_PAYLOAD(&s_parse_msg),
                       s_parse_msg.len);
                s_rx_ready = 1U;
                s_rx_frame_count++;
                s_last_rx_ms = now_ms;
            ...
            if (s_parse_status.parse_error != parse_error_before) {
                uint8_t delta = (uint8_t)(
                    s_parse_status.parse_error - parse_error_before);
                s_parse_error_count += delta;
                s_crc_error_count += delta;
            }
            if (s_parse_status.packet_rx_drop_count != drop_before) {
                s_rx_drop_count += (uint16_t)(
                    s_parse_status.packet_rx_drop_count - drop_before);
            }
        }
    }
...
Lora_Result_t Lora_E22_CopyRxFrame(Lora_RxFrame_t *out)
{
    ...
    if (out == 0) {
        return LORA_RESULT_INVALID_PARAM;
    }
                               ...
                               uint32_t offline_timeout_ms)
{
    ...
    if (((s_last_rx_ms != 0U) &&
         ((uint32_t)(now_ms - s_last_rx_ms) <=
          offline_timeout_ms)) ||
        ((s_last_tx_ms != 0U) &&
         ((uint32_t)(now_ms - s_last_tx_ms) <=
          offline_timeout_ms))) {
...
static void Lora_E22_TxStep(uint32_t now_ms)
{
    ...
    case LORA_TX_WAIT_AUX:
        ...
        if ((s_tx_state == LORA_TX_WAIT_AUX) &&
            ((uint32_t)(now_ms - s_tx_state_ms) >
             LORA_E22_TX_AUX_TIMEOUT_MS)) {
            s_tx_busy_count++;
            s_tx_state = LORA_TX_IDLE;
        }
        break;
    ...
    case LORA_TX_SENDING:
        if (BSP_LoRa_IsTxBusy() == 0U) {
            ...
            s_last_tx_ms = now_ms;
            s_tx_state = LORA_TX_IDLE;
        } else if ((uint32_t)(now_ms - s_tx_state_ms) >
                   LORA_E22_TX_DMA_TIMEOUT_MS) {
            BSP_LoRa_AbortTx();
            s_send_error_count++;
            s_tx_state = LORA_TX_IDLE;
        }
...
{
    if (data == 0 || len == 0U || len > LORA_E22_TX_BUF_SIZE) {
        return LORA_RESULT_INVALID_PARAM;
    }
