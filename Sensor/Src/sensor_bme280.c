...
#define BME280_CTRL_MEAS_FORCED_X1 \
    ((BME280_OSRS_X1 << 5) | (BME280_OSRS_X1 << 2) | BME280_MODE_FORCED)
...
static BSP_Status_t Bme280_ReadReg(uint8_t reg,
                                   uint8_t *data,
                                   uint16_t len)
{
    return BSP_I2C_MemRead(s_addr,
                           reg,
                           data,
                           len,
...
{
    return BSP_I2C_MemWrite(s_addr,
                            reg,
                            &value,
                            1U,
                            BME280_I2C_TIMEOUT_MS);
...
{
    if (BSP_I2C_IsDeviceReady(BME280_I2C_ADDR_PRIMARY,
                              BME280_I2C_TIMEOUT_MS) == BSP_STATUS_OK)
    {
        s_addr = BME280_I2C_ADDR_PRIMARY;
        return BME280_RESULT_OK;
    }

    if (BSP_I2C_IsDeviceReady(BME280_I2C_ADDR_SECONDARY,
                              BME280_I2C_TIMEOUT_MS) == BSP_STATUS_OK)
    {
...
static Bme280_Result_t Bme280_CheckChipId(void)
{
    ...
    if (Bme280_ReadReg(BME280_REG_ID, &chip_id, 1U) != BSP_STATUS_OK)
    {
        return BME280_RESULT_IO_ERROR;
    }
...
static Bme280_Result_t Bme280_WaitStatusClear(uint8_t mask)
{
    ...
    for (i = 0U; i < BME280_MAX_WAIT_POLLS; ++i)
    {
        ...
        if (Bme280_ReadReg(BME280_REG_STATUS, &status, 1U) !=
            BSP_STATUS_OK)
        {
            return BME280_RESULT_IO_ERROR;
        }
...
static Bme280_Result_t Bme280_ReadCalibration(void)
{
    ...
    if ((Bme280_ReadReg(BME280_REG_CALIB_TP,
                        tp,
                        sizeof(tp)) != BSP_STATUS_OK) ||
        (Bme280_ReadReg(BME280_REG_CALIB_H1,
                        &h1,
                        1U) != BSP_STATUS_OK) ||
        (Bme280_ReadReg(BME280_REG_CALIB_H2,
                        h,
                        sizeof(h)) != BSP_STATUS_OK))
    {
        return BME280_RESULT_IO_ERROR;
    }
    ...
    s_calib.dig_H4 =
        Bme280_SignExtend12(
            (uint16_t)(((uint16_t)h[3] << 4) |
                       (h[4] & 0x0FU)));
    s_calib.dig_H5 =
        Bme280_SignExtend12(
            (uint16_t)(((uint16_t)h[5] << 4) |
                       (h[4] >> 4)));
    s_calib.dig_H6 = (int8_t)h[6];

    return (s_calib.dig_P1 != 0U)
               ? BME280_RESULT_OK
...
static float Bme280_CompensateTemperature(int32_t adc_T)
{
    ...
    var1 = ((((adc_T >> 3) -
              ((int32_t)s_calib.dig_T1 << 1))) *
            ((int32_t)s_calib.dig_T2)) >>
           11;
    var2 = (((((adc_T >> 4) -
               ((int32_t)s_calib.dig_T1)) *
              ((adc_T >> 4) -
               ((int32_t)s_calib.dig_T1))) >>
             12) *
...
{
    if ((Bme280_WriteReg(BME280_REG_CTRL_HUM,
                         BME280_CTRL_HUM_X1) != BSP_STATUS_OK) ||
        (Bme280_WriteReg(BME280_REG_CONFIG,
                         BME280_CONFIG_STANDBY_0_5MS) != BSP_STATUS_OK) ||
        (Bme280_WriteReg(BME280_REG_CTRL_MEAS,
                         BME280_CTRL_MEAS_FORCED_X1) != BSP_STATUS_OK))
    {
        return BME280_RESULT_IO_ERROR;
    }
...
Bme280_Result_t Sensor_BME280_Init(void)
{
    ...
    if (Bme280_WriteReg(BME280_REG_RESET,
                        BME280_RESET_CMD) != BSP_STATUS_OK)
    {
        return BME280_RESULT_IO_ERROR;
    }
...
Bme280_Result_t Sensor_BME280_Service(uint32_t now_ms)
{
    ...
    if (s_initialized == 0U)
    {
         ...
         */
        if ((s_next_init_ms != 0U) && (now_ms < s_next_init_ms))
        {
    ...
    if (Bme280_ReadReg(BME280_REG_DATA,
                       data,
                       sizeof(data)) != BSP_STATUS_OK)
    {
        Bme280_NoteReadFailure();
        return BME280_RESULT_IO_ERROR;
    }
    ...
    s_snapshot.pressure_pa =
        Bme280_CompensatePressurePa(adc_P);
    s_snapshot.relative_humidity_pct =
...
Bme280_Result_t Sensor_BME280_CopySnapshot(Bme280_Snapshot_t *out)
{
    if (out == 0)
    {
        return BME280_RESULT_INVALID_PARAM;
    }
