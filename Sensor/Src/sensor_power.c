...
static uint8_t Power_CalcPercent(uint32_t voltage_mv)
{
    ...
    percent = (((voltage_mv - POWER_EMPTY_MV) * 100U) +
               (range_mv / 2U)) /
              range_mv;
...
static uint8_t Power_ApplyPercentHysteresis(
    uint8_t previous,
    uint8_t candidate)
{
    if (candidate > (uint8_t)(previous + POWER_PERCENT_HYSTERESIS))
    {
        return candidate;
    }
    if (previous > (uint8_t)(candidate + POWER_PERCENT_HYSTERESIS))
    {
        return candidate;
    }
    return previous;
...
Power_Result_t Sensor_Power_Service(uint32_t now_ms)
{
    ...
    s_snapshot.percent =
        (s_snapshot.rx_sequence == 1U)
            ? candidate_percent
            : Power_ApplyPercentHysteresis(
                  s_snapshot.percent,
                  candidate_percent);
    s_snapshot.low_voltage =
        (voltage_mv <= POWER_EMPTY_MV) ? 1U : 0U;
