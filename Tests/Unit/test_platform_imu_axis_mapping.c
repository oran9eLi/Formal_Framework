...
int main(void)
{
    ...
    ok &= ExpectI32("accel y inverted for roll", accel_mg[1], -200);
    ok &= ExpectI32("accel z unchanged", accel_mg[2], 300);
    ok &= ExpectI32("gyro x inverted for roll", gyro_mdps[0], -400);
    ok &= ExpectI32("gyro y unchanged for pitch", gyro_mdps[1], 500);
    ok &= ExpectI32("gyro z inverted for yaw", gyro_mdps[2], -600);
