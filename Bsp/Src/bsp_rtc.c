...
static uint8_t BSP_RTC_DaysInMonth(uint16_t year, uint8_t month)
{
  ...
  if (month == 2U) { return (BSP_RTC_IsLeapYear(year) != 0U) ? 29U : 28U; }
 ...
 */
static uint8_t BSP_RTC_IsDateTimeValid(const BSP_RTC_DateTime_t *date_time)
{
  ...
  if ((date_time->day < 1U) || (date_time->day > BSP_RTC_DaysInMonth(date_time->year, date_time->month))) { return 0U; }
 ...
 */
static uint32_t BSP_RTC_DaysSince2000(uint16_t year, uint8_t month, uint8_t day)
{
  uint32_t days = 0U;
  uint16_t y;
  uint8_t m;

  for (y = 2000U; y < year; y++) { days += (BSP_RTC_IsLeapYear(y) != 0U) ? 366U : 365U; }
  for (m = 1U; m < month; m++) { days += BSP_RTC_DaysInMonth(year, m); }
 ...
 */
static uint8_t BSP_RTC_ComputeWeekday(uint16_t year, uint8_t month, uint8_t day)
{
  uint32_t days;
 ...
 */
static BSP_Status_t BSP_RTC_WaitClockFlag(uint32_t flag, FlagStatus expected)
{
  uint32_t start_ms = HAL_GetTick();

  while (__HAL_RCC_GET_FLAG(flag) != expected) {
    if ((uint32_t)(HAL_GetTick() - start_ms) > BSP_RTC_CLOCK_STARTUP_TIMEOUT_MS) { return BSP_STATUS_TIMEOUT; }
  }
...
static BSP_Status_t BSP_RTC_SelectLsiClock(void)
{
  ...
  if (BSP_RTC_WaitClockFlag(RCC_FLAG_LSIRDY, SET) != BSP_STATUS_OK) { return BSP_STATUS_TIMEOUT; }
...
static BSP_Status_t BSP_RTC_ConfigClock(void)
{
  ...
  if (current_source == RCC_RTCCLKSOURCE_LSE) {
    ...
    if (BSP_RTC_SelectLseClock() != BSP_STATUS_OK) {
      if (BSP_RTC_SelectLsiClock() != BSP_STATUS_OK) { return BSP_STATUS_ERROR; }
    }
  }
...
static void BSP_RTC_FillHandle(void)
{
  ...
  s_rtc.Init.SynchPrediv    = (s_rtc_clock_source == RCC_RTCCLKSOURCE_LSI) ? 249U : 255U;
...
{
  if (s_rtc_ready == 0U) { return 0U; }

  return (HAL_RTCEx_BKUPRead(&s_rtc, BSP_RTC_BACKUP_MAGIC_REGISTER) == BSP_RTC_MAGIC_VALUE) ? 1U : 0U;
}
...
BSP_Status_t BSP_RTC_ReadDateTime(BSP_RTC_DateTime_t *out)
{
  ...
  if ((out == 0) || (s_rtc_ready == 0U)) { return BSP_STATUS_ERROR; }
  if (BSP_RTC_IsTimeValid() == 0U) { return BSP_STATUS_ERROR; }

  memset(&date, 0, sizeof(date));
  memset(&time, 0, sizeof(time));
  if (HAL_RTC_GetTime(&s_rtc, &time, RTC_FORMAT_BIN) != HAL_OK) { return BSP_STATUS_ERROR; }
  if (HAL_RTC_GetDate(&s_rtc, &date, RTC_FORMAT_BIN) != HAL_OK) { return BSP_STATUS_ERROR; }
  ...
  return (BSP_RTC_IsDateTimeValid(out) != 0U) ? BSP_STATUS_OK : BSP_STATUS_ERROR;
}
...
BSP_Status_t BSP_RTC_WriteDateTime(const BSP_RTC_DateTime_t *date_time)
{
  ...
  if ((s_rtc_ready == 0U) || (BSP_RTC_IsDateTimeValid(date_time) == 0U)) { return BSP_STATUS_ERROR; }
  ...
  date.WeekDay = BSP_RTC_ComputeWeekday(date_time->year, date_time->month, date_time->day);

  if (HAL_RTC_SetTime(&s_rtc, &time, RTC_FORMAT_BIN) != HAL_OK) { return BSP_STATUS_ERROR; }
  if (HAL_RTC_SetDate(&s_rtc, &date, RTC_FORMAT_BIN) != HAL_OK) { return BSP_STATUS_ERROR; }

  HAL_RTCEx_BKUPWrite(&s_rtc, BSP_RTC_BACKUP_MAGIC_REGISTER, BSP_RTC_MAGIC_VALUE);
  return BSP_STATUS_OK;
}
