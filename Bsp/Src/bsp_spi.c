...
static BSP_Status_t BSP_SPI1_Configure(uint32_t prescaler)
{
  ...
  if ((s_initialized != 0U) && (s_current_prescaler == prescaler)) { return BSP_STATUS_OK; }
  ...
  if (s_initialized != 0U) { (void)HAL_SPI_DeInit(&s_hspi1); }
...
{
  if ((tx == 0) || (length == 0U)) { return BSP_STATUS_ERROR; }
  if (BSP_SPI1_EnsureReady() != BSP_STATUS_OK) { return BSP_STATUS_ERROR; }
  return BSP_SPI1_MapHalStatus(HAL_SPI_Transmit(&s_hspi1, (uint8_t *)tx, length, timeout_ms));
}
...
{
  if ((rx == 0) || (length == 0U)) { return BSP_STATUS_ERROR; }
  if (BSP_SPI1_EnsureReady() != BSP_STATUS_OK) { return BSP_STATUS_ERROR; }
  return BSP_SPI1_MapHalStatus(HAL_SPI_Receive(&s_hspi1, rx, length, timeout_ms));
}
...
{
  if ((tx == 0) || (rx == 0) || (length == 0U)) { return BSP_STATUS_ERROR; }
  if (BSP_SPI1_EnsureReady() != BSP_STATUS_OK) { return BSP_STATUS_ERROR; }
  return BSP_SPI1_MapHalStatus(HAL_SPI_TransmitReceive(&s_hspi1, (uint8_t *)tx, rx, length, timeout_ms));
}
