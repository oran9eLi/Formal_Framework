...
static const uint8_t *Display_GfxGetGlyph(char ch)
{
  ...
  if ((ch >= '0') && (ch <= '9')) {
    return s_gfx_font_5x7[(uint8_t)(ch - '0')];
  }
  ...
  if ((ch >= 'A') && (ch <= 'Z')) {
    return s_gfx_font_5x7[10U + (uint8_t)(ch - 'A')];
  }
...
{
  if (s_gfx_ready == 0U) {
    return DISPLAY_GFX_NOT_READY;
  }

  if (Display_GfxIsPointInScreen(x, y) == 0U) {
...
static void Display_GfxPlotCirclePoints(uint16_t x0, uint16_t y0, int32_t x, int32_t y, uint16_t color)
{
  ...
  px = (int32_t)x0 + x;
  py = (int32_t)y0 + y;
  if ((px >= 0) && (py >= 0) && (px < (int32_t)DISPLAY_GFX_WIDTH) && (py < (int32_t)DISPLAY_GFX_HEIGHT)) {
    (void)Display_GfxWritePixel((uint16_t)px, (uint16_t)py, color);
  }
...
uint16_t Display_GfxMakeRgb565(uint8_t red, uint8_t green, uint8_t blue)
{
  ...
  color = (uint16_t)(((uint16_t)(red & 0xF8U) << 8) |
...
Display_GfxResult_t Display_GfxDrawChar(uint16_t x, uint16_t y, char ch, uint16_t color, uint8_t scale)
{
  ...
  if (((uint32_t)x + (5U * scale)) > DISPLAY_GFX_WIDTH ||
      ((uint32_t)y + (7U * scale)) > DISPLAY_GFX_HEIGHT) {
    return DISPLAY_GFX_PARAM_ERROR;
  }
  ...
  for (col = 0U; col < 5U; col++) {
    for (row = 0U; row < 7U; row++) {
      if ((glyph[col] & (uint8_t)(1U << row)) != 0U) {
        (void)Display_GfxFillRect((uint16_t)(x + ((uint16_t)col * scale)),
...
Display_GfxResult_t Display_GfxDrawString(uint16_t x, uint16_t y, const char *text, uint16_t color, uint8_t scale)
{
  ...
  while (*text != '\0') {
    if (Display_GfxDrawChar(cursor_x, y, *text, color, scale) != DISPLAY_GFX_OK) {
      return DISPLAY_GFX_PARAM_ERROR;
    }
...
Display_GfxResult_t Display_GfxFillRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color)
{
  ...
  if ((width == 0U) || (height == 0U) || (x >= DISPLAY_GFX_WIDTH) || (y >= DISPLAY_GFX_HEIGHT)) {
    return DISPLAY_GFX_PARAM_ERROR;
  }

  clipped_width = width;
  clipped_height = height;
  if (((uint32_t)x + clipped_width) > DISPLAY_GFX_WIDTH) {
    clipped_width = (uint16_t)(DISPLAY_GFX_WIDTH - x);
  }
  if (((uint32_t)y + clipped_height) > DISPLAY_GFX_HEIGHT) {
    clipped_height = (uint16_t)(DISPLAY_GFX_HEIGHT - y);
  }
  ...
  for (row = 0U; row < clipped_height; row++) {
    for (col = 0U; col < clipped_width; col++) {
      s_gfx_port.draw_pixel((uint16_t)(x + col), (uint16_t)(y + row), color);
    }
  }
...
Display_GfxResult_t Display_GfxDrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
  ...
  if ((Display_GfxIsPointInScreen(x1, y1) == 0U) || (Display_GfxIsPointInScreen(x2, y2) == 0U)) {
    return DISPLAY_GFX_PARAM_ERROR;
  }
...
Display_GfxResult_t Display_GfxDrawRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color)
{
  ...
  if ((width < 2U) || (height < 2U) || (x >= DISPLAY_GFX_WIDTH) || (y >= DISPLAY_GFX_HEIGHT) ||
      (((uint32_t)x + width) > DISPLAY_GFX_WIDTH) ||
      (((uint32_t)y + height) > DISPLAY_GFX_HEIGHT)) {
    return DISPLAY_GFX_PARAM_ERROR;
  }

  result = Display_GfxDrawHLine(x, y, width, color);
  if (result != DISPLAY_GFX_OK) {
    return result;
  }
...
Display_GfxResult_t Display_GfxDrawCircle(uint16_t x0, uint16_t y0, uint16_t radius, uint16_t color)
{
  ...
  if ((radius == 0U) || (Display_GfxIsPointInScreen(x0, y0) == 0U)) {
    return DISPLAY_GFX_PARAM_ERROR;
  }
...
Display_GfxResult_t Display_GfxFillCircle(uint16_t x0, uint16_t y0, uint16_t radius, uint16_t color)
{
  ...
  while (x <= y) {
    ...
    top = (int32_t)y0 + y;
    if ((left >= 0) && (top >= 0) && (top < (int32_t)DISPLAY_GFX_HEIGHT)) {
      (void)Display_GfxDrawHLine((uint16_t)left, (uint16_t)top, length, color);
    }
    top = (int32_t)y0 - y;
    if ((left >= 0) && (top >= 0) && (top < (int32_t)DISPLAY_GFX_HEIGHT)) {
      (void)Display_GfxDrawHLine((uint16_t)left, (uint16_t)top, length, color);
                                               ...
                                               uint16_t border_color)
{
  ...
  if ((width < 4U) || (height < 4U) || (max_value == 0U)) {
    return DISPLAY_GFX_PARAM_ERROR;
  }
                                        ...
                                        uint16_t color)
{
  ...
  if ((width == 0U) || (height == 0U) || (cell_width == 0U) || (cell_height == 0U)) {
    return DISPLAY_GFX_PARAM_ERROR;
  }
  ...
  for (offset = 0U; offset <= width; offset = (uint16_t)(offset + cell_width)) {
    ...
    if ((uint16_t)(width - offset) < cell_width) {
      break;
    }
  }
  ...
  for (offset = 0U; offset <= height; offset = (uint16_t)(offset + cell_height)) {
    ...
    if ((uint16_t)(height - offset) < cell_height) {
      break;
    }
  }
